    ...
    <div class="max-w-3xl mx-auto px-4 py-6">
        <div class="mb-4">
            <a href="{{ route('categories.index') }}"
                class="t-secondary hover:text-[#fbbf24] transition text-sm font-medium">
        ...
        @if($category->monthly_budget > 0)
            ...
            <div class="card p-5 mb-4">
                ...
                <div class="w-full bg-gray-200 dark:bg-white/10 rounded-full h-3 overflow-hidden">
                    <div class="{{ $barColor }} h-3 rounded-full transition-all duration-500" style="width: {{ $budgetPct }}%">
                    </div>
                </div>
        ...
        <div class="card p-5">
                        ...
                        <li class="py-3 flex flex-col sm:flex-row sm:justify-between gap-1">
                            ...
                            <div class="sm:text-right">
                                <div class="font-semibold t-primary">
                                    @money($currentTeam->convertToDefaultCurrency($t->amount, $t->currency, $t->created_at))
                                    {{ $currencySymbol }}
                                </div>
            ...
            @else
                <div class="t-secondary text-sm">{{ __('No expenses or incomes in this category.') }}</div>
            @endif
        </div>
    </div>
@endsection